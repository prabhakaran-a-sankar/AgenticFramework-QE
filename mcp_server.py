...
from quality_engineering_agentic_framework.agents.standalone_automation_agent import StandaloneAutomationAgent
...
@tool_if_enabled("generate_test_cases")
...
@tool_if_enabled("generate_test_scripts")
...
@tool_if_enabled("generate_test_data")
...
@tool_if_enabled("generate_api_test_cases")
...
@tool_if_enabled("add_requirement_document")
...
async def standalone_automation(
    ...
    if file_map or config_text:
        discovered = StandaloneAutomationAgent.discover_config(
            files=file_map, config_text=config_text or "")
    else:
        discovered = StandaloneAutomationAgent.discover_config(project_path=project_path)
    logger.info(
    ...
    agent = StandaloneAutomationAgent(llm, {
    ...
    if server_can_write:
        ...
        if confirmed:
            ...
            write_report = StandaloneAutomationAgent.write_files(result, project_path)
