...
def display_results(data, story_key=None):
    ...
    with col1:
        ...
        st.markdown(f"<h2 style='margin-top: 0;'>{data.get('predicted_bugs', 0)}</h2>", unsafe_allow_html=True)
    
    with col2:
        risk = data.get('risk_level', 'UNKNOWN')
        risk_colors = {'CRITICAL': '#dc2626', 'HIGH': '#ea580c', 'MEDIUM': '#f59e0b', 'LOW': '#10b981'}
        color = risk_colors.get(risk, '#6b7280')
        st.markdown("**Risk Level**")
        st.markdown(f"<h2 style='margin-top: 0; color: {color};'>{risk}</h2>", unsafe_allow_html=True)
    
    with col3:
        regression_score = round(100 - abs(data.get('defect_probability', 50) - 50), 1)
        st.markdown("**Regression Score**")
        st.markdown(f"<h2 style='margin-top: 0;'>{regression_score}%</h2>", unsafe_allow_html=True)
    
    st.markdown("<br>", unsafe_allow_html=True)
    
    col_left, col_center, col_right = st.columns([1, 2, 1])
    with col_center:
        st.markdown("<p style='text-align: center; font-weight: bold; margin-bottom: 10px;'>Defect Probability</p>", unsafe_allow_html=True)
...
def render_defect_predictor_tab():
    st.header("Defect Predictor - AI-Powered Bug Prediction")
